...
class M_Fina_Aktiva_Pembelian extends CI_Model {
    ...
    function create()
    {
        ...
        // Insert Header Trans
        $data_header = array(
                        'susumber' => $this->M_transaksi->prefixtrans(element('Fina_Aktiva_Pembelian',NID)),
                        'sutanggal' => tgl_database($_POST['tgl']),                        
                        'sunotransaksi' => $nomor,
                        'sukontak' => $_POST['kontak'],
                        'sutotaltransaksi' => 0,    
                        'sujenis' => 2,
                        'suuraian' => 'Pembelian aktiva tetap',
                        'sucoa' => $_POST['coa'],
        ...
        foreach($data as $item){
            ...
            $detil = array(
                    'sdidsu' => $id,
                    'sdurutan' => $r,
                    'sdsumber' => $this->M_transaksi->prefixtrans(element('Fina_Aktiva_Pembelian',NID)),                    
                    'sditem' => $aktiva['id'],
                    'sdmasuk' => $aktiva['qty'],
                    'sdmasukd' => $aktiva['qty'],                    
            ...
            $r++;     
            $totalsaldo += $aktiva['harga'];       
        }

        $this->db->where('suid', $id);
        $this->db->update('fasetu',array('sutotaltransaksi' => $totalsaldo));        

        $sql="CALL SP_JURNAL_PEMBELIAN_AKTIVA_ADD(".$id.")";
        $this->db->query($sql);

        // USERLOG
        $uactivity = _anomor(element('Fina_Aktiva_Pembelian',NID));
    ...
    {
        $nomor = 0;
        $nomor1 = $this->M_transaksi->prefixtrans(element('Fina_Aktiva_Pembelian',NID));
