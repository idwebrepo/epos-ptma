<?php
    ...
    $query  = "SELECT A.suid 'id',A.sunotransaksi 'nomor',DATE_FORMAT(A.sutanggal,'%d-%m-%Y') 'tanggal',B.knama 'kontak',
                      ...
                      D.gnama 'asal', D.gnama 'tujuan', E.jnama 'jenis'                         
                 FROM fstoku A 
            ...
            LEFT JOIN bjenispenyesuaian E ON A.sujenispenyesuaian=E.jid 
                WHERE A.suid = '".$id."'";

    $header = $CI->M_transaksi->get_data_query($query);
    $header = json_decode($header);	

    $querydtl  = "SELECT B.ikode 'noitem',B.inama 'item',A.sdmasuk 'qtyin',A.sdkeluar 'qtyout',C.skode 'satuan',A.sdcatatan 'catatan',
    					 D.gnama 'gudang' 
                 FROM fstokd A 
...
<div class="header-report">
	...
	<div class="left">
		<table class="table" style="margin-top: 5px;">
			<tbody>
				<tr>
					<td align="center" class="border-1 bg-dark" width="50%">Jenis Penyesuaian :</td>
					<td>&nbsp;</td>
					<td align="center" class="border-1 bg-dark" width="50%">Kontak :</td>
				</tr>
				<tr>
					<td align="center" class="border-1 px-1 py-1"><?= $jenis ?></td>
					<td>&nbsp;</td>
...
<div class="content-report">
	<table class="table table-border">
		...
		<tbody>
			...
			?>
				<tr>
					...
					<td class="right px-1 py-1"><?= $row->qtyin ?></td>
					<td class="right px-1 py-1"><?= $row->qtyout ?></td>												
		...
		<tfoot>
			...
			<tr>
				<td colspan="4" class="left py-1"></td>
				<th colspan="2" class="right px-1 py-1">Total Masuk</th>
				<th class="right px-1 py-1"><?= $qtyin ?></th>				
			</tr>							
			<tr>
				<td colspan="4" class="left py-1"></td>
				<th colspan="2" class="right px-1 py-1">Total Keluar</th>
				<th class="right px-1 py-1"><?= $qtyout ?></th>				
			</tr>										
		</tfoot>
	</table>	
