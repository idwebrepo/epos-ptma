<?php
    ...
    $transcode2 = $CI->M_transaksi->prefixtrans(element('PJ_Penjualan_Tunai',NID));            
    $transcode3 = $CI->M_transaksi->prefixtrans(element('SV_Penjualan_Tunai',NID));            
    $query  = "SELECT A.ipuid 'id',A.ipunotransaksi 'nomor',DATE_FORMAT(A.iputanggal,'%d-%m-%Y') 'tanggal',
                          B.knama 'kontak', A.ipuuraian 'uraian', 
                          D.ikode 'kditem', D.inama 'item', C.ipdkeluar 'qty',
                          C.ipdharga 'harga', C.ipddiskon 'diskon',
                          (C.ipdharga-C.ipddiskon)*C.ipdkeluar 'subtotal'
