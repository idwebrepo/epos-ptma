...
$transcode3 = $CI->M_transaksi->prefixtrans(element('PJ_Faktur_Penjualan', NID));
$query  = "SELECT A.ipuid 'id', A.ipusumber 'sumber', A.ipunotransaksi 'nomor',DATE_FORMAT(A.iputanggal,'%d-%m-%Y') 'tanggal',
                          B.knama 'kontak', A.ipuuraian 'uraian', 
                          D.ikode 'kditem', D.inama 'item', D.ihargabeli 'hargabeli',C.sdkeluar 'qty',
                ...
                LEFT JOIN fstokh E ON C.sdid=E.shidsd
	                WHERE A.ipusumber IN ('" . $transcode1 . "', '" . $transcode2 . "', '" . $transcode3 . "')  
	...
	<table class="table">
		...
		<tbody>
			...
			foreach ($datareport->data as $row) {
				...
				if($row->hpp == 0){
					$totalHpp = $row->hargabeli * $row->qty;
				}
				

				if ($row->sumber == $transcode1 || $row->sumber == $transcode2) {
				...
				echo "<td class='right'>" . eFormatNumber($totalHpp, 2) . "</td>";
				echo "<td class='right'>" . eFormatNumber($row->subtotal - $totalHpp, 2) . "</td>";
				echo "</tr>";
