...
class PB_Keluar_Barang extends CI_Controller {
   ...
   function __construct() { 
      ...
      $this->load->model('M_PB_Keluar_Barang');
   }

   function savedata(){
      if($_POST['id']==''){
        echo $this->M_PB_Keluar_Barang->tambahTransaksi();
      }else{
        echo $this->M_PB_Keluar_Barang->ubahTransaksi();      
      }
   }

   function deletedata(){
      echo $this->M_PB_Keluar_Barang->hapusTransaksi();          
   }
   ...
   function getdata(){
      ...
      $transcode = $this->M_transaksi->prefixtrans(element('PB_Keluar_Barang',NID));        
   		$query = "SELECT A.suid 'id', A.sunotransaksi 'nomor', DATE_FORMAT(A.sutanggal,'%d-%m-%Y') 'tanggal',
                       ...
                       A.susouid 'orderid', I.sounotransaksi 'nomororder', A.sugudangasal 'idgudang', N.gnama 'gudang',
               ...
               LEFT JOIN bgudang N ON A.sugudangasal=N.gid               
   ...
   function getdatapenerimaan(){
      ...
      $query = "SELECT A.sditem 'item', C.inama 'namaitem', 
                       (A.sdmasuk-(SELECT IFNULL(SUM(sdkeluar),0) FROM fstokd WHERE sdpbdid=A.sdid)) 'qty', 
                       A.sdharga 'harga', 
                       A.sddiskon 'diskon', 
                       A.sddiskonpersen 'persen',
                       ((A.sdharga-A.sddiskon)*(A.sdmasuk-(SELECT IFNULL(SUM(sdkeluar),0) FROM fstokd WHERE sdpbdid=A.sdid))) 'jumlah', 
