...
class Fina_Bank_Keluar extends CI_Controller {
   ...
   function deletedatamulti(){
    echo $this->M_Fina_Bank_Keluar->hapusTransaksiMulti();          
   }
   ...
   function getdata(){
      ...
      $transcode = $this->M_transaksi->prefixtrans($transcode);        
   		$query = "SELECT A.cuid 'id', A.cunotransaksi 'nomor', 
               ...
               IF(D.cddebit > 0, D.cddebit, IF(D.cdkredit = 0, 0, D.cdkredit * -1)) 'jmldetil', 
               IF(D.cddebitv > 0, D.cddebitv, IF(D.cdkreditv = 0, 0, D.cdkreditv * -1)) 'jmldetilv',
