...
$(function () {
  ...
  tabel=$('#table').DataTable({
    ...
    "dom": '<"top"pi>tr<"clear">',
    "ajax": {
        "url":base_url+"Datatable_Master/view_jenis_penyesuaian",
        "type":"post",
        "data": function(data){
          data.kode = $('#kode').val();
          data.nama = $('#nama').val();
...
$("#badd").click(function() { 
    $.ajax({ 
      "url"    : base_url+"Modal/form_jenis_penyesuaian", 
      "type"   : "POST", 
      "dataType" : "html",
      "beforeSend": function(){
        parent.window.$(".loader-wrap").removeClass("d-none");      
        parent.window.$(".modal").modal("show");                  
        parent.window.$(".modal-title").html("Tambah Jenis Penyesuaian");
        parent.window.$("#modaltrigger").val("iframe-page-jenispenyesuaian");        
      },     
      "error": function(){
        parent.window.$(".loader-wrap").addClass("d-none");            
        console.log('error menampilkan form jenis penyesuaian...');
        return;
      },
...
$("#bedit").click(function() {
    ...
    $.ajax({ 
      ...
      "beforeSend": function(){      
        parent.window.$(".loader-wrap").removeClass("d-none");
        parent.window.$(".modal").modal("show");                  
        parent.window.$(".modal-title").html("Jenis Penyesuaian");
        parent.window.$("#modaltrigger").val("iframe-page-jenispenyesuaian");        
      },     
      "error": function(){
        parent.window.$(".loader-wrap").addClass("d-none");      
...
var _deleteData = (function(){
    ...
    $.ajax({ 
      "url"    : base_url+"Master_Jenis_Penyesuaian/deletedata", 
      "type"   : "POST", 
      "data"   : "id="+id,
      "cache"    : false,
      "beforeSend" : function(){
      ...
      "success": function(result) {
        parent.window.$(".loader-wrap").addClass("d-none");        
        if(result=='sukses'){
          parent.window.toastr.success("Data jenis penyesuaian berhasil dihapus");                  
          _reloaddatatable();
          return;
        } else {        
