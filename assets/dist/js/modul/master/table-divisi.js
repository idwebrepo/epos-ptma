...
$(function () {
  ...
  tabel=$('#divisi-table').DataTable({
    ...
    "dom": '<"top"pi>tr<"clear">',
    "ajax": {
        "url":base_url+"Datatable_Master/view_table_divisi",
        "type":"post",
        "data": function(data){
          data.kode = $('#kode').val();
          data.nama = $('#nama').val();
...
$("#badd").click(function() { 
  $.ajax({ 
    "url"    : base_url+"Modal/form_divisi", 
    "type"   : "POST", 
    "dataType" : "html",
    "beforeSend": function(){
      parent.window.$(".loader-wrap").removeClass("d-none");      
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Tambah Divisi");
      parent.window.$("#modaltrigger").val("iframe-page-divisi");              
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");            
      console.log('error menampilkan form divisi...');
      return;
    },
...
$("#bdelete").click(function() {
  ...
  const id = $('#divisi-table').DataTable().cell($('#divisi-table').DataTable().rows({selected:true}),2).data();
...
$("#bedit").click(function() {
  const id = $('#divisi-table').DataTable().cell($('#divisi-table').DataTable().rows({selected:true}),0).data();
  ...
  $.ajax({ 
    ...
    "beforeSend": function(){      
      parent.window.$(".loader-wrap").removeClass("d-none");
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Data Divisi");
      parent.window.$("#modaltrigger").val("iframe-page-divisi");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");      
...
$('#divisi-table').on('dblclick','tr',function(e){
    e.preventDefault();
    e.stopPropagation();
...
var _deleteData = (function(){
  ...
  $.ajax({ 
    "url"    : base_url+"Master_Divisi/deletedata", 
    "type"   : "POST", 
    "data"   : "id="+id,
    "cache"    : false,
    "beforeSend" : function(){
    ...
    "success": function(result) {
      parent.window.$(".loader-wrap").addClass("d-none");        
      if(result=='sukses'){
        parent.window.toastr.success("Data divisi berhasil dihapus");                  
        _reloaddatatable();
        return;
      } else {        
...
$("#bfilter").click(function() {
  if($("#fDataTable").hasClass("d-none")){
    $("#divisi-table").removeClass("w-100");
    $("#divisi-table").addClass("w-75");
    $("#fDataTable").removeClass("d-none");
        $(".noresultfound-x").css("background-position","30% 160px");                                   
  }else {
    $("#divisi-table").removeClass("w-75");
    $("#divisi-table").addClass("w-100");
...
$("#submitfilter").click(function() {
  $('#divisi-table').DataTable().ajax.reload();  
