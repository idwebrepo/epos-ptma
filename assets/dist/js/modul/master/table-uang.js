...
$(function () {
  ...
  tabel=$('#uang-table').DataTable({
    ...
    "dom": '<"top"pi>tr<"clear">',
    "ajax": {
        "url":base_url+"Datatable_Master/view_table_uang",
        "type":"post",
        "data": function(data){
          data.kode = $('#kode').val();
          data.nama = $('#nama').val();
...
$("#badd").click(function() { 
  $.ajax({ 
    "url"    : base_url+"Modal/form_uang", 
    "type"   : "POST", 
    "dataType" : "html",
    "beforeSend": function(){
      parent.window.$(".loader-wrap").removeClass("d-none");      
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Tambah Mata Uang");
      parent.window.$("#modaltrigger").val("iframe-page-muang");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");            
      console.log('error menampilkan modal form uang...');
      return;
    },
...
$("#bdelete").click(function() {
  const id = $('#uang-table').DataTable().cell($('#uang-table').DataTable().rows({selected:true}),2).data();
...
$("#bedit").click(function() {
  const id = $('#uang-table').DataTable().cell($('#uang-table').DataTable().rows({selected:true}),0).data();
  ...
  $.ajax({ 
    ...
    "beforeSend": function(){      
      parent.window.$(".loader-wrap").removeClass("d-none");
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Mata Uang");
      parent.window.$("#modaltrigger").val("iframe-page-muang");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");      
      console.log('error menampilkan form mata uang...');
      return;
    },
    "success": async function(result) {
      await parent.window.$(".main-modal-body").html(result);
...
$('#uang-table').on('dblclick','tr',function(e){
    e.preventDefault();
    e.stopPropagation();
...
var _deleteData = (function(){
  ...
  $.ajax({ 
    "url"    : base_url+"Master_Uang/deletedata", 
    "type"   : "POST", 
    "data"   : "id="+id,
    "cache"    : false,
    "beforeSend" : function(){
    ...
    "success": function(result) {
      parent.window.$(".loader-wrap").addClass("d-none");        
      if(result=='sukses'){
        parent.window.toastr.success("Data mata uang berhasil dihapus");                  
        _reloaddatatable();
        return;
      } else {        
...
$("#bfilter").click(function() {
  if($("#fDataTable").hasClass("d-none")){
    $("#uang-table").removeClass("w-100");
    $("#uang-table").addClass("w-75");
    $("#fDataTable").removeClass("d-none");
        $(".noresultfound-x").css("background-position","30% 160px");                                   
  }else {
    $("#uang-table").removeClass("w-75");
    $("#uang-table").addClass("w-100");
...
$("#submitfilter").click(function() {
  $('#uang-table').DataTable().ajax.reload();  
