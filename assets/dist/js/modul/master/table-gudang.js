...
$("#badd").click(function() { 
  $.ajax({ 
    "url"    : base_url+"Modal/form_gudang", 
    "type"   : "POST", 
    "dataType" : "html",
    "beforeSend": function(){
      parent.window.$(".loader-wrap").removeClass("d-none");      
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Tambah Gudang");
      parent.window.$("#modaltrigger").val("iframe-page-gudang");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");            
      console.log('error menampilkan form gudang...');
      return;
    },
...
$("#bdelete").click(function() {
  const id = $('#gudang-table').DataTable().cell($('#gudang-table').DataTable().rows({selected:true}),2).data();
...
$("#bedit").click(function() {
  ...
  $.ajax({ 
    ...
    "beforeSend": function(){      
      parent.window.$(".loader-wrap").removeClass("d-none");
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Data Gudang");
      parent.window.$("#modaltrigger").val("iframe-page-gudang");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");      
...
var _deleteData = (function(){
  ...
  $.ajax({ 
    "url"    : base_url+"Master_Gudang/deletedata", 
    "type"   : "POST", 
    "data"   : "id="+id,
    "cache"    : false,
    "beforeSend" : function(){
    ...
    "success": function(result) {
      parent.window.$(".loader-wrap").addClass("d-none");        
      if(result=='sukses'){
        parent.window.toastr.success("Data gudang berhasil dihapus");                  
        _reloaddatatable();
        return;
      } else {        
...
$("#bfilter").click(function() {
  if($("#fDataTable").hasClass("d-none")){
    $("#gudang-table").removeClass("w-100");
    $("#gudang-table").addClass("w-75");
    $("#fDataTable").removeClass("d-none");
        $(".noresultfound-x").css("background-position","30% 160px");                                   
  }else {
    $("#gudang-table").removeClass("w-75");
    $("#gudang-table").addClass("w-100");
...
$("#submitfilter").click(function() {
  $('#gudang-table').DataTable().ajax.reload();  
