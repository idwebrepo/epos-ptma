...
$(function () {
  ...
  tabel=$('#ckontak-table').DataTable({
    ...
    "dom": '<"top"pi>tr<"clear">',
    "ajax": {
        "url":base_url+"Datatable_Master/view_table_ckontak",
        "type":"post",
        "data": function(data){
          data.nama = $('#nama').val();
        }                           
...
$("#badd").click(function() { 
  $.ajax({ 
    "url"    : base_url+"Modal/form_kategori_kontak", 
    "type"   : "POST", 
    "dataType" : "html",
    "beforeSend": function(){
      parent.window.$(".loader-wrap").removeClass("d-none");      
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Tambah Kategori Kontak");
      parent.window.$("#modaltrigger").val("iframe-page-ckontak");              
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");            
      console.log('error menampilkan form kategori kontak...');
      return;
    },
...
$("#bdelete").click(function() {
  ...
  const id = $('#ckontak-table').DataTable().cell($('#ckontak-table').DataTable().rows({selected:true}),2).data();
...
$("#bedit").click(function() {
  const id = $('#ckontak-table').DataTable().cell($('#ckontak-table').DataTable().rows({selected:true}),0).data();
  ...
  $.ajax({ 
    ...
    "beforeSend": function(){      
      parent.window.$(".loader-wrap").removeClass("d-none");
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Data Kategori Kontak");
      parent.window.$("#modaltrigger").val("iframe-page-ckontak");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");      
...
$('#ckontak-table').on('dblclick','tr',function(e){
    e.preventDefault();
    e.stopPropagation();
...
var _deleteData = (function(){
  ...
  $.ajax({ 
    "url"    : base_url+"Master_Kategori_Kontak/deletedata", 
    "type"   : "POST", 
    "data"   : "id="+id,
    "cache"    : false,
    "beforeSend" : function(){
    ...
    "success": function(result) {
      parent.window.$(".loader-wrap").addClass("d-none");        
      if(result=='sukses'){
        parent.window.toastr.success("Data kategori kontak berhasil dihapus");                  
        _reloaddatatable();
        return;
      } else {        
...
$("#bfilter").click(function() {
  if($("#fDataTable").hasClass("d-none")){
    $("#ckontak-table").removeClass("w-100");
    $("#ckontak-table").addClass("w-75");
    $("#fDataTable").removeClass("d-none");
        $(".noresultfound-x").css("background-position","30% 160px");                                   
  }else {
    $("#ckontak-table").removeClass("w-75");
    $("#ckontak-table").addClass("w-100");
...
$("#submitfilter").click(function() {
  $('#ckontak-table').DataTable().ajax.reload();  
