...
var _saveData = function () {
	...
	rey.set("id", id);
	rey.set("nama", nama);
	// rey.set("kdunit", kdunit);

	$.ajax({
		url: base_url + "Master_Kategori_Item/savedata",
		type: "POST",
		data: rey,
		processData: false,
		contentType: false,
		...
		success: function (result) {
			...
			if (result == "sukses") {
				$("#modal").modal("hide");
				toastr.success("Data kategori item berhasil disimpan");
				try {
					$("#iframe-page-kategoriitem")
						.contents()
						.find("#submitfilter")
						.click();
				} catch (err) {
...
function _getData(id) {
	...
	$.ajax({
		url: base_url + "Master_Kategori_Item/getdata",
		type: "POST",
		dataType: "json",
		data: "id=" + id,
		cache: false,
