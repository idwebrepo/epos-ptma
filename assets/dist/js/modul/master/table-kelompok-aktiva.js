...
$(function () {
  ...
  tabel=$('#fatipe-table').DataTable({
    ...
    "dom": '<"top"pi>tr<"clear">',
    "ajax": {
        "url":base_url+"Datatable_Master/view_table_fatipe",
        "type":"post",
        "data": function(data){
          data.kode = $('#kode').val();
          data.nama = $('#nama').val();
...
$("#badd").click(function() { 
  ...
  $.ajax({ 
    "url"    : base_url+"Modal/form_kelompok_aktiva", 
    "type"   : "POST", 
    "dataType" : "html",
    "beforeSend": function(){
      parent.window.$(".loader-wrap").removeClass("d-none");      
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Tambah Kelompok Aktiva");
      parent.window.$("#modaltrigger").val("iframe-page-fatipe");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");            
      console.log('error menampilkan form kelompok aktiva...');
      return;
    },
...
$("#bdelete").click(function() {
  ...
  const id = $('#fatipe-table').DataTable().cell($('#fatipe-table').DataTable().rows({selected:true}),2).data();
...
$("#bedit").click(function() {
  const id = $('#fatipe-table').DataTable().cell($('#fatipe-table').DataTable().rows({selected:true}),0).data();
  ...
  $.ajax({ 
    ...
    "beforeSend": function(){      
      parent.window.$(".loader-wrap").removeClass("d-none");
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Kelompok Aktiva");
      parent.window.$("#modaltrigger").val("iframe-page-fatipe");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");      
...
$('#fatipe-table').on('dblclick','tr',function(e){
    e.preventDefault();
    e.stopPropagation();
...
var _deleteData = (function(){
  ...
  $.ajax({ 
    "url"    : base_url+"Master_Kelompok_Aktiva/deletedata", 
    "type"   : "POST", 
    "data"   : "id="+id,
    "cache"    : false,
    "beforeSend" : function(){
    ...
    "success": function(result) {
      parent.window.$(".loader-wrap").addClass("d-none");        
      if(result=='sukses'){
        parent.window.toastr.success("Data kelompok aktiva berhasil dihapus");                  
        _reloaddatatable();
        return;
      } else {        
...
$("#bfilter").click(function() {
  if($("#fDataTable").hasClass("d-none")){
    $("#fatipe-table").removeClass("w-100");
    $("#fatipe-table").addClass("w-75");
    $("#fDataTable").removeClass("d-none");
        $(".noresultfound-x").css("background-position","30% 160px");                                   
  }else {
    $("#fatipe-table").removeClass("w-75");
    $("#fatipe-table").addClass("w-100");
...
$("#submitfilter").click(function() {
  $('#fatipe-table').DataTable().ajax.reload();  
