...
$(function () {
  ...
  tabel=$('#proyek-table').DataTable({
    ...
    "dom": '<"top"pi>tr<"clear">',
    "ajax": {
        "url":base_url+"Datatable_Master/view_table_proyek",
        "type":"post",
        "data": function(data){
          data.kode = $('#kode').val();
          data.nama = $('#nama').val();
...
$("#badd").click(function() { 
  $.ajax({ 
    "url"    : base_url+"Modal/form_proyek", 
    "type"   : "POST", 
    "dataType" : "html",
    "beforeSend": function(){
      parent.window.$(".loader-wrap").removeClass("d-none");      
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Tambah Proyek");
      parent.window.$("#modaltrigger").val("iframe-page-proyek");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");            
      console.log('error menampilkan form proyek...');
      return;
    },
...
$("#bdelete").click(function() {
  ...
  const id = $('#proyek-table').DataTable().cell($('#proyek-table').DataTable().rows({selected:true}),2).data();
...
$("#bedit").click(function() {
  const id = $('#proyek-table').DataTable().cell($('#proyek-table').DataTable().rows({selected:true}),0).data();
  ...
  $.ajax({ 
    ...
    "beforeSend": function(){      
      parent.window.$(".loader-wrap").removeClass("d-none");
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Data Proyek");
      parent.window.$("#modaltrigger").val("iframe-page-proyek");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");      
...
$('#proyek-table').on('dblclick','tr',function(e){
    e.preventDefault();
    e.stopPropagation();
...
var _deleteData = (function(){
  ...
  $.ajax({ 
    "url"    : base_url+"Master_Proyek/deletedata", 
    "type"   : "POST", 
    "data"   : "id="+id,
    "cache"    : false,
    "beforeSend" : function(){
    ...
    "success": function(result) {
      parent.window.$(".loader-wrap").addClass("d-none");        
      if(result=='sukses'){
        parent.window.toastr.success("Data proyek berhasil dihapus");                  
        _reloaddatatable();
        return;
      } else {        
...
$("#bfilter").click(function() {
  if($("#fDataTable").hasClass("d-none")){
    $("#proyek-table").removeClass("w-100");
    $("#proyek-table").addClass("w-75");
    $("#fDataTable").removeClass("d-none");
        $(".noresultfound-x").css("background-position","30% 160px");                                   
  }else {
    $("#proyek-table").removeClass("w-75");
    $("#proyek-table").addClass("w-100");
...
$("#submitfilter").click(function() {
  $('#proyek-table').DataTable().ajax.reload();  
