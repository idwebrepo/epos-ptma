...
$(function () {
  ...
  tabel=$('#bank-table').DataTable({
    ...
    "dom": '<"top"pi>tr<"clear">',
    "ajax": {
        "url":base_url+"Datatable_Master/view_table_bank",
        "type":"post",
        "data": function(data){
          data.kode = $('#kode').val();
          data.nama = $('#nama').val();
...
$("#badd").click(function() { 
  $.ajax({ 
    ...
    "beforeSend": function(){
      ...
      parent.window.$(".modal-title").html("Tambah Bank");
      parent.window.$("#modaltrigger").val("iframe-page-mbank");              
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");            
      console.log('error menampilkan modal form bank...');
      return;
    },
...
var _deleteData = (function(){
  ...
  $.ajax({ 
    "url"    : base_url+"Master_Bank/deletedata", 
    "type"   : "POST", 
    "data"   : "id="+id,
    "cache"    : false,
    "beforeSend" : function(){
    ...
    "success": function(result) {
      parent.window.$(".loader-wrap").addClass("d-none");        
      if(result=='sukses'){
        parent.window.toastr.success("Data bank berhasil dihapus");                  
        _reloaddatatable();
        return;
      } else {        
