...
$(function () {
  ...
  tabel=$('#citem-table').DataTable({
    ...
    "dom": '<"top"pi>tr<"clear">',
    "ajax": {
        "url":base_url+"Datatable_Master/view_table_citem",
        "type":"post",
        "data": function(data){
          data.nama = $('#nama').val();
        }                           
...
$("#badd").click(function() { 
  $.ajax({ 
    "url"    : base_url+"Modal/form_kategori_item", 
    "type"   : "POST", 
    "dataType" : "html",
    "beforeSend": function(){
      parent.window.$(".loader-wrap").removeClass("d-none");      
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Tambah Kategori Item");
      parent.window.$("#modaltrigger").val("iframe-page-kategoriitem");              
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");            
      console.log('error menampilkan form kategori item...');
      return;
    },
...
$("#bdelete").click(function() {
  ...
  const id = $('#citem-table').DataTable().cell($('#citem-table').DataTable().rows({selected:true}),2).data();
...
$("#bedit").click(function() {
  const id = $('#citem-table').DataTable().cell($('#citem-table').DataTable().rows({selected:true}),0).data();
  ...
  $.ajax({ 
    ...
    "beforeSend": function(){      
      parent.window.$(".loader-wrap").removeClass("d-none");
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Data Kategori Item");
      parent.window.$("#modaltrigger").val("iframe-page-kategoriitem");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");      
...
$('#citem-table').on('dblclick','tr',function(e){
    e.preventDefault();
    e.stopPropagation();
...
var _deleteData = (function(){
  ...
  $.ajax({ 
    "url"    : base_url+"Master_Kategori_Item/deletedata", 
    "type"   : "POST", 
    "data"   : "id="+id,
    "cache"    : false,
    "beforeSend" : function(){
    ...
    "success": function(result) {
      parent.window.$(".loader-wrap").addClass("d-none");        
      if(result=='sukses'){
        parent.window.toastr.success("Data kategori item berhasil dihapus");                  
        _reloaddatatable();
        return;
      } else {        
...
$("#bfilter").click(function() {
  if($("#fDataTable").hasClass("d-none")){
    $("#citem-table").removeClass("w-100");
    $("#citem-table").addClass("w-75");
    $("#fDataTable").removeClass("d-none");
        $(".noresultfound-x").css("background-position","30% 160px");                                   
  }else {
    $("#citem-table").removeClass("w-75");
    $("#citem-table").addClass("w-100");
...
$("#submitfilter").click(function() {
  $('#citem-table').DataTable().ajax.reload();  
