...
$(function () {
  ...
  tabel=$('#termin-table').DataTable({
    ...
    "dom": '<"top"pi>tr<"clear">',
    "ajax": {
        "url":base_url+"Datatable_Master/view_table_termin",
        "type":"post",
        "data": function(data){
          data.kode = $('#kode').val();
          data.nama = $('#nama').val();
...
$("#badd").click(function() { 
  $.ajax({ 
    "url"    : base_url+"Modal/form_termin", 
    "type"   : "POST", 
    "dataType" : "html",
    "beforeSend": function(){
      parent.window.$(".loader-wrap").removeClass("d-none");      
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Tambah Termin");
      parent.window.$("#modaltrigger").val("iframe-page-termin");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");            
      console.log('error menampilkan form termin...');
      return;
    },
...
$("#bdelete").click(function() {
  const id = $('#termin-table').DataTable().cell($('#termin-table').DataTable().rows({selected:true}),2).data();
...
$("#bedit").click(function() {
  const id = $('#termin-table').DataTable().cell($('#termin-table').DataTable().rows({selected:true}),0).data();
  ...
  $.ajax({ 
    ...
    "beforeSend": function(){      
      parent.window.$(".loader-wrap").removeClass("d-none");
      parent.window.$(".modal").modal("show");                  
      parent.window.$(".modal-title").html("Data Termin");
      parent.window.$("#modaltrigger").val("iframe-page-termin");        
    },     
    "error": function(){
      parent.window.$(".loader-wrap").addClass("d-none");      
...
$('#termin-table').on('dblclick','tr',function(e){
    e.preventDefault();
    e.stopPropagation();
...
var _deleteData = (function(){
  ...
  $.ajax({ 
    "url"    : base_url+"Master_Termin/deletedata", 
    "type"   : "POST", 
    "data"   : "id="+id,
    "cache"    : false,
    "beforeSend" : function(){
    ...
    "success": function(result) {
      parent.window.$(".loader-wrap").addClass("d-none");        
      if(result=='sukses'){
        parent.window.toastr.success("Data termin berhasil dihapus");                  
        _reloaddatatable();
        return;
      } else {        
...
$("#bfilter").click(function() {
  if($("#fDataTable").hasClass("d-none")){
    $("#termin-table").removeClass("w-100");
    $("#termin-table").addClass("w-75");
    $("#fDataTable").removeClass("d-none");
        $(".noresultfound-x").css("background-position","30% 160px");                                   
  }else {
    $("#termin-table").removeClass("w-75");
    $("#termin-table").addClass("w-100");
