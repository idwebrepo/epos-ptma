...
$(function() {
	...
	$("#bprint").click(() => {
		...
		if(totalcek>0){
			...
			data = data.slice(0,-1);
			// window.open(`${base_url}Laporan/multipreview/page-fpj/${data}`);
			window.open(`${base_url}Laporan/multipreview/page-fpjn/${data}`);
		} else {		
			const id = $('#table').DataTable().cell($('#table').DataTable().rows({selected:true}),0).data();		
			...
			window.open(`${base_url}Laporan/preview/page-fpjn/${id}`);
		}
	});	
		...
		if(totalcek>0){
			parent.window.Swal.fire({
				...
				if (result.isConfirmed) {
				    ...
				    $.ajax({ 
				        "url"    : base_url+"PJ_Faktur_Penjualan/deletedataduamulti", 
				        "type"   : "POST", 
				        "data"   : rey,
				        "processData": false,
				        "contentType": false,
	...
	var _sendemail = (emailto, data) => {
			...
			$.ajax({ 
			"url"    : base_url+"Laporan/multimail/page-fpj/"+emailto+"/"+data, 
			"type"   : "POST", 
			"data"   : rey,
			"processData": false,
			"contentType": false,
