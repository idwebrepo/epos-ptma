...
tabeltransaksi=$('#transaksi-table').DataTable({
  ...
  "ajax": {
      "url":base_url+"Datatable_Transaksi/view_data_service",
      "type":"post",
      "beforeSend": function(){
...
function restable(){
  ...
  $('#modal').modal('hide');        
  $("#"+trigger).contents().find("#idservicetrigger").val("").trigger('change');  
  $("#"+trigger).contents().find("#idservicetrigger").val(id).trigger('change');
  return;        
}