/* ========================================================================================== */
/* File Name : penjualan-tunai-service.js
/* Info Lain : 
/* ========================================================================================== */
...
$(function() {
  ...
  setupHiddenInputChangeListener($('#idservicetrigger')[0]);    
...
//    alert(this.id);
  ...
  $("#bTable").click(function() {
    ...
    location.href=base_url+"page/pos2Data";      
  });
  ...
  $("#blistservice").click(function() {

      $.ajax({ 
        "url"    : base_url+"Modal/cari_data_service", 
        "type"   : "POST", 
        "dataType" : "html", 
        "beforeSend": function(){
          parent.window.$('.loader-wrap').removeClass('d-none');
          ...
          parent.window.$(".modal-header").addClass("bg-info"); 
          parent.window.$(".modal").modal("show");                  
          parent.window.$(".modal-title").html("Data Service");
          parent.window.$("#modaltrigger").val("iframe-page-pos2");
          parent.window.$('#coltrigger').val('idservice');                
        },
        "error": function(){
          parent.window.$('.loader-wrap').addClass('d-none');
          console.log('error menampilkan modal data service...');
          return;
        },
        "success": function(result) {
          parent.window.$(".main-modal-body").html(result);
  ...
  $("#blisttunda").click(function() {
      ...
      $.ajax({ 
        "url"    : base_url+"Modal/cari_tunda_pos_service", 
        "type"   : "POST", 
        "dataType" : "html", 
        "beforeSend": function(){
          parent.window.$('.loader-wrap').removeClass('d-none');
  ...
  $("#btunda").click(function() {
    ...
    if($('#idkaryawan').val()=='' || $('#idkaryawan').val()==0){
    ...
    $.ajax({ 
      "url"    : base_url+"SV_Penjualan_Tunai/savedatatunda", 
      "type"   : "POST", 
      "data"   : rey,
      "processData": false,
      "contentType": false,
  ...
  $("#bpembayaran").click(function() {
      ...
      if(Number($("#ttrans").val().split('.').join('').toString().replace(',','.'))==0) {
        ...
        $.ajax({ 
          "url"    : base_url+"Modal/pembayaran_pos_service", 
          "type"   : "POST", 
          "dataType" : "html", 
          "beforeSend": function(){
            parent.window.$('.loader-wrap').removeClass('d-none');
  ...
  $("#bprint").click(() => {
      if($('#id').val()=='') return;    
      window.open(`${base_url}Laporan/preview/page-pos2/${$("#id").val()}`)    
  });
  ...
  $('#idservicetrigger').on('change',function(){
    var idtrans = $(this).val();
    _getDataService(idtrans);
  });  
  ...
  $(this).on("select2:select", "select[name^='item']", function(){
      ...
      $.ajax({ 
        "url"    : base_url+"SV_Penjualan_Tunai/get_item", 
        "type"   : "POST", 
...
//  Component_Select2('.satuan',`${base_url}Select_Master/view_satuan`,null,null);  
  Component_Select2_Item('.item',`${base_url}Select_Master/view_item`,null,null);
...
var _getlistitem = () => {
  ...
  $.ajax({ 
    "url"    : base_url+"Datatable_Master/view_item_pos_service",       
    "type"   : "POST", 
    "dataType" : "json", 
...
//      alert(result.data[0]['nama']);
      $.each(result.data, function() {
        ...
        html = `<button id="${result.data[row]['id']}" type="button" value="${result.data[row]['kode']}" title="${result.data[row]['kode']}  ${result.data[row]['nama']}" class="btn-iteml btn btn-light mx-1 my-2" style="height: 140px">
...
var _tambahItem = (id) => {
      ...
      $.ajax({ 
        "url"    : base_url+"SV_Penjualan_Tunai/get_item_kode", 
        "type"   : "POST", 
...
var _cetakStruk = (nomor) => {
      ...
      if(opt==0){
        $("#hasilPdf").attr('src',`${base_url}Laporan/preview/page-pos2/${nomor}`);
        setTimeout(function(){
        ...
        parent.window.$("#modaltrigger").val("iframe-page-pos2");
        parent.window.$(".main-modal-body").html(`<object id="pStruk" data="${base_url}Laporan/preview/page-pos2/${nomor}#toolbar=1&navpanes=1&zoom=150%" type="application/pdf" width="100%" height="500"></object>`);
      }
}
...
var _saveData = () => {
  ...
  $.ajax({ 
    "url"    : base_url+"SV_Penjualan_Tunai/savedata", 
    "type"   : "POST", 
    "data"   : rey,
    "processData": false,
    "contentType": false,
...
var _getDataService = (id) => {

  if(id=='' || id==null) return;

  $.ajax({ 
    "url"    : base_url+"SV_Penjualan_Tunai/getdataservice",       
    "type"   : "POST", 
    "dataType" : "json", 
    "data" : "id="+id,
    "cache"  : false,
    ...
    "error"  : function(){
      parent.window.toastr.error('Error : Gagal mengambil data service !');
      parent.window.$('.loader-wrap').addClass('d-none');                  
      return;
    },
    "success" : async function(result){
...
var _getDataTunda = (id) => {
  ...
  $.ajax({ 
    "url"    : base_url+"SV_Penjualan_Tunai/getdatatunda",       
    "type"   : "POST", 
    "dataType" : "json", 
    "data" : "id="+id,
    "cache"  : false,
    ...
    "error"  : function(){
      parent.window.toastr.error('Error : Gagal mengambil data tunda pos service !');
      parent.window.$('.loader-wrap').addClass('d-none');                  
      return;
    },
    "success" : function(result){
      if (typeof result.pesan !== 'undefined') {
        ...
        $('#tgl').datepicker('setDate',result.data[0]['tanggal']);
        $('#idkontak').val(result.data[0]['kontakid']);
        $('#idservice').val(result.data[0]['idservice']);        
        $('#kontak').val(result.data[0]['kontakkode']);
        $('#namakontak').html(result.data[0]['kontak']);        
        $('#idkaryawan').val(result.data[0]['idkaryawan']);
        $('#karyawan').val(result.data[0]['karyawan']);        
...
var _getDataTransaksi = (id) => {
  ...
  $.ajax({ 
    "url"    : base_url+"SV_Penjualan_Tunai/getdata",       
    "type"   : "POST", 
    "dataType" : "json", 
    "data" : "id="+id,
    "cache"  : false,
