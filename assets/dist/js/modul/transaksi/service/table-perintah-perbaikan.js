/* ========================================================================================== */
/* File Name : table-perintah-perbaikan.js
/* Info Lain : 
/* ========================================================================================== */
...
$(function() {
	...
	tabel=$('#table').DataTable({
		...
		"dom": '<"top"pi>tr<"clear">',
		"ajax": {
		    "url":base_url+"Datatable_Transaksi_Full/view_perintah_perbaikan",
		    "type":"post",
	        "data": function(data){
	...
	$("#badd").click(function() {
		...
		location.href=base_url+"page/sppuk";      
	});
	...
	$("#bedit").click(function() {
		...
		parent.window.$('.loader-wrap').removeClass('d-none');
		location.href=base_url+"page/sppuk/?id="+id;      
	});
		...
		if(totalcek>0){
			parent.window.Swal.fire({
				...
				if (result.isConfirmed) {
				    ...
				    $.ajax({ 
				        "url"    : base_url+"SV_Perintah_Perbaikan/deletedatamulti", 
				        "type"   : "POST", 
				        "data"   : rey,
				        "processData": false,
				        "contentType": false,
	...
	$("#bprint").click(() => {
		...
		if(totalcek>0){
			...
			data = data.slice(0,-1);
			window.open(`${base_url}Laporan/multipreview/page-sppuk/${data}`);
		} else {		
			const id = $('#table').DataTable().cell($('#table').DataTable().rows({selected:true}),0).data();		

	        if(typeof id=='undefined') return;

			window.open(`${base_url}Laporan/preview/page-sppuk/${id}`);
		}
	});	
	...
	var _sendemail = (emailto, data) => {
			...
			$.ajax({ 
			"url"    : base_url+"Laporan/multimail/page-sppuk/"+emailto+"/"+data, 
			"type"   : "POST", 
			"data"   : rey,
			"processData": false,
			"contentType": false,
