...
$(function() {
	...
	tabel=$('#table').DataTable({
		...
		"dom": '<"top"pi>tr<"clear">',
		"ajax": {
		    "url":base_url+"Datatable_Transaksi_Full/view_kas_keluar",
		    "type":"post",
	        "data": (data) => {
	...
	$("#badd").click(() => {
		...
		location.href=base_url+"page/bkk";      
	});
	...
	$("#bedit").click(() => {
		...
		parent.window.$('.loader-wrap').removeClass('d-none');
		location.href=base_url+"page/bkk/?id="+id;      
	});
	...
	$("#bprint").click(() => {
		...
		if(totalcek>0){
			...
			data = data.slice(0,-1);
			window.open(`${base_url}Laporan/multipreview/page-bkk/${data}`);
		} else {		
			const id = $('#table').DataTable().cell($('#table').DataTable().rows({selected:true}),0).data();		

	        if(typeof id=='undefined') return;

			window.open(`${base_url}Laporan/preview/page-bkk/${id}`);
		}
	});	
		...
		if(totalcek>0){
			parent.window.Swal.fire({
				...
				if (result.isConfirmed) {
				    ...
				    $.ajax({ 
				        "url"    : base_url+"Fina_Kas_Keluar/deletedatamulti", 
				        "type"   : "POST", 
				        "data"   : rey,
				        "processData": false,
				        "contentType": false,
	...
	var _sendemail = (emailto, data) => {
			...
			$.ajax({ 
			"url"    : base_url+"Laporan/multimail/page-bkk/"+emailto+"/"+data, 
			"type"   : "POST", 
			"data"   : rey,
			"processData": false,
			"contentType": false,
