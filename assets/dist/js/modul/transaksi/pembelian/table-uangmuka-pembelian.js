...
$( function() {
	...
	$("#bprint").click(() => {
		...
		if(totalcek>0){
			...
			data = data.slice(0,-1);
			window.open(`${base_url}Laporan/multipreview/page-umb/${data}`)
		} else {
	...
	var _sendemail = (emailto, data) => {
			...
			$.ajax({ 
			"url"    : base_url+"Laporan/multimail/page-umb/"+emailto+"/"+data, 
			"type"   : "POST", 
			"data"   : rey,
			"processData": false,
			"contentType": false,
		...
		if(totalcek>0){
			parent.window.Swal.fire({
				...
				if (result.isConfirmed) {
				    ...
				    $.ajax({ 
				        "url"    : base_url+"PB_Uang_Muka_Pembelian/deletedatamulti", 
				        "type"   : "POST", 
				        "data"   : rey,
				        "processData": false,
				        "contentType": false,
