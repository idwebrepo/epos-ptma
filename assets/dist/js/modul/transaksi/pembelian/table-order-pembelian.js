...
$(function() {
		...
		if(totalcek>0){
			parent.window.Swal.fire({
				...
				if (result.isConfirmed) {
				    ...
				    $.ajax({ 
				        "url"    : base_url+"PB_Order_Pembelian/deletedatamulti", 
				        "type"   : "POST", 
				        "data"   : rey,
				        "processData": false,
				        "contentType": false,
	...
	$("#bprint").click(() => {
		...
		if(totalcek>0){
			...
			data = data.slice(0,-1);
			window.open(`${base_url}Laporan/multipreview/page-opb/${data}`)
		} else {
...
//						window.open(`${base_url}Laporan/multimail/page-opb/${emailto}/${data}`)
					}
				})
			} else {
				_sendemail(emailto,data);
...
//			window.open(`${base_url}Laporan/multimail/page-opb/${emailto}/${id}`);
		}
	});	
	...
	var _sendemail = (emailto, data) => {
			...
			$.ajax({ 
			"url"    : base_url+"Laporan/multimail/page-opb/"+emailto+"/"+data, 
			"type"   : "POST", 
			"data"   : rey,
			"processData": false,
			"contentType": false,
