...
if (activeMenu=='iframe-page-pos') {
  ...
  $("#bpilihkontak").removeClass("btn-primary");
  $("#bpilihkontak").addClass("btn-secondary"); 
//  $("#bAddKontak").addClass("d-none");   
  ...
  $("#bpilihkontak").removeClass("btn-primary");
  $("#bpilihkontak").addClass("btn-info"); 
//  $("#bAddKontak").addClass("d-none");   
  $("th").addClass("bg-info");   
}
...
$("#bAddKontak").click(function() {
    ...
    $.ajax({ 
        ...
        "beforeSend": () => {
            ...
            parent.window.$("#modalfilter .modal-title").html("Tambah Kontak");
        },         
        "error": () => {
            parent.window.$(".loader-wrap").addClass("d-none");                    
            parent.window.toastr.error('Error menampilkan modal form kontak...');
            return;
        },
        "success": (result) => {
...
function restable(){
  const id = $('#contact-table').DataTable().cell($('#contact-table').DataTable().rows({selected:true}),0).data(),
      ...
      nama = $('#contact-table').DataTable().cell($('#contact-table').DataTable().rows({selected:true}),4).data(),
      alamat = $('#contact-table').DataTable().cell($('#contact-table').DataTable().rows({selected:true}),6).data(),     
...
function sincron(idkontak){
  console.log(idkontak);
  $.ajax({ 
    url    : base_url+"Client/fetch_person", 
    type   : "POST", 
    data   : {
            'nis'        : idkontak,
    },    
